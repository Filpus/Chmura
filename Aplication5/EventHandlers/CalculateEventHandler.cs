    ...
    public class CalculateEventHandler : IEventHandler<CalculateDmgEvent>
    {
        ...
        public async Task Handle(CalculateDmgEvent @event)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Weapon details: Name={Name}, DamageDice={DamageDice}, DamageType={DamageType}", weapon.Name, weapon.DamageDice, weapon.DamageType);

                // Rzut na obrażenia
                Random random = new Random();
                int baseDamage = RollDice(weapon.DamageDice, weapon.DamageDiceCount, random);
                int totalDamage = baseDamage + character.Strength;

                // Uwzględnienie odporności i wrażliwości
